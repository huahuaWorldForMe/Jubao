 ...
 * 操作成功返回函数
 *----------------------------------------
 * @param  string $lang_key 操作提示信息，填写语言文件的键值
 * @param  string $data 跳转为URL地址,ajax为要返回的数据
 * @param  string $second_ajax 跳转为等待时间,ajax输入“ajax”通过ajax返回
 * @param  string $type ajax返回时返回类型 格式：JSON/XML/EVAL/JSONP
 *----------------------------------------
 * @author Nzing
 * @access public
 * @return json
 */
function success($lang_key, $data = '', $type = 'JSON')
{
    $info = $lang_key;
    $result = array();
    $result['message'] = $info;
 ...
 * @param  string $data 跳转为URL地址,ajax为要返回的数据
 * @param  string $second_ajax 跳转为等待时间,ajax输入“ajax”通过ajax返回
 * @param  string $un_lan_key 不调用语言文件
 * @param  string $show_error_code 显示错误代码
 * @param  string $type ajax返回时返回类型 格式：JSON/XML/EVAL/JSONP
 *----------------------------------------
 * @author Nzing
 * @access public
 * @return json
 */
function error($lang_key, $data = '', $second_ajax = '2', $un_lan_key = TRUE, $show_error_code = FALSE, $type = 'JSON')
{
    if ($un_lan_key === TRUE) {
        $error_info = $lang_key;
    } else {
        $this->lang->load('error');
        $error_info = $this->lang->line($lang_key);
        if ($show_error_code === TRUE) {
...
function ajaxReturn($data, $type = 'JSON')
{
    ...
    switch (strtoupper($type)) {
        ...
        case 'JSONP' :
            if (strpos($_SERVER["HTTP_USER_AGENT"], "MSIE7.0") || strpos($_SERVER["HTTP_USER_AGENT"], "MSIE6.0")) {
                header('Content-Type:text/html; charset=utf-8');
                // 返回JSON数据格式到客户端 包含状态信息
                exit('jsonpcallback' . '(' . json_encode($data) . ')');
            } else {
                // 返回JSON数据格式到客户端 包含状态信息
                header('Content-Type:application/json; charset=utf-8');
                exit('jsonpcallback' . '(' . json_encode($data) . ')');
...
function jubao_log($jubao_id,$content){
    $log = array(
        'jubao_id' => $jubao_id,
        'log_content' =>$content,
        'log_time' => time()
    );
    Db::name('jubao_log')->insert($log);//打日志
}
//=======end api
